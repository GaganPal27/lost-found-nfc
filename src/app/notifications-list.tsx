...
import { View, Text, FlatList, TouchableOpacity, RefreshControl, Platform, UIManager, LayoutAnimation, StatusBar, Animated, Alert, StyleSheet } from 'react-native';
...
// Default export: full standalone screen (bell icon → /notifications-list)
  ...
  return (
    <View style={notifStyles.root}>
      <LinearGradient
        ...
        style={[notifStyles.header, { paddingTop: insets.top + 12 }]}
      >
        <View style={notifStyles.hDecor1} />
        <View style={notifStyles.hDecor2} />
        <View style={notifStyles.headerRow}>
          <TouchableOpacity onPress={() => router.back()} style={notifStyles.backBtn} activeOpacity={0.8}>
            <Feather name="arrow-left" size={20} color="#ffffff" />
          </TouchableOpacity>
          <View style={{ flex: 1, marginLeft: 12 }}>
            <Text style={notifStyles.headerSup}>ALERTS</Text>
            <Text style={notifStyles.headerTitle}>Notifications</Text>
          </View>
        </View>
      </LinearGradient>
      <NotificationsList bottomPadding={60} />
    </View>
  );
}
...
const notifStyles = StyleSheet.create({
  ...
  hDecor1: { position: 'absolute', top: -30, right: -20, width: 130, height: 130, borderRadius: 65, backgroundColor: 'rgba(255,255,255,0.07)' },
  hDecor2: { position: 'absolute', bottom: -40, left: -30, width: 110, height: 110, borderRadius: 55, backgroundColor: 'rgba(255,255,255,0.05)' },
  headerRow: { flexDirection: 'row', alignItems: 'center' },
  backBtn: { width: 36, height: 36, borderRadius: 18, backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center' },
  headerSup: { color: 'rgba(255,255,255,0.7)', fontSize: 10, fontWeight: '800', letterSpacing: 2, textTransform: 'uppercase' },
