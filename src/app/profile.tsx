    ...
    <View className="flex-1 bg-slate-50">
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: tabBarClearance }}>
        ...
        <Animated.View style={{ opacity: fadeIn }}>
          {/* Avatar Card */}
          <View className="mx-6 bg-white border border-slate-200 rounded-3xl p-6 mb-5 items-center shadow-sm relative overflow-hidden">
            {(dbUser?.successful_recoveries || 0) > 0 && (
              <View className="absolute top-4 right-4 bg-yellow-100 border border-yellow-300 px-3 py-1.5 rounded-full flex-row items-center z-10">
            ...
            {editingName ? (
              <View className="w-full flex-row items-center border-b border-slate-300 pb-1 mb-2">
                ...
                />
                <TouchableOpacity onPress={handleSaveName} disabled={savingName} className="ml-2 overflow-hidden rounded-full">
                  <LinearGradient colors={['#6366f1', '#7c3aed']} start={{x:0,y:0}} end={{x:1,y:1}} className="px-4 py-1.5">
                    {savingName ? <ActivityIndicator size="small" color="#fff" /> : <Text className="text-white font-bold text-xs">SAVE</Text>}
                  </LinearGradient>
                </TouchableOpacity>
              </View>
            ) : (
          ...
          <View className="mx-6 bg-white border border-slate-200 rounded-3xl p-6 mb-5 shadow-sm">
            ...
            {maxItems !== Infinity && (
              <View className="h-2 bg-slate-100 rounded-full overflow-hidden mb-4">
                <LinearGradient
                  colors={['#6366f1', '#7c3aed']} start={{x:0,y:0}} end={{x:1,y:0}}
            ...
            {tier !== 'max' && (
              <TouchableOpacity
                ...
                className="rounded-2xl mt-2 overflow-hidden shadow-sm"
                activeOpacity={0.88}
              >
                <LinearGradient colors={['#6366f1', '#7c3aed']} start={{x:0,y:0}} end={{x:1,y:1}} className="p-4 items-center">
                  <Text className="text-white font-bold tracking-wide">↑ Upgrade Plan</Text>
                </LinearGradient>
              </TouchableOpacity>
            )}
          </View>
          ...
          <View className="mx-6 bg-white border border-slate-200 rounded-3xl p-6 mb-5 shadow-sm">
            ...
            >
              ...
              {tier === 'pro' || tier === 'max' ? (
                <View className="flex-row items-center bg-indigo-100 border border-indigo-200 px-3 py-1.5 rounded-full">
                  <View className="w-2 h-2 rounded-full bg-indigo-500 mr-2" />
                  <Text className="text-indigo-700 text-xs font-bold tracking-wider">ACTIVE</Text>
                </View>
              ) : (
          ...
          <View className="mx-6 bg-white border border-slate-200 rounded-3xl p-6 mb-5 shadow-sm">
            ...
            {(PLAN_FEATURES[tier as keyof typeof PLAN_FEATURES] || PLAN_FEATURES.basic).map((feat, i) => (
              <View key={i} className="flex-row items-center py-2">
                <Text className="text-indigo-600 font-bold mr-3">✓</Text>
          ...
          {useAuthStore.getState().isAdmin && (
            <TouchableOpacity
              ...
              className="mx-6 rounded-2xl mb-5 shadow-sm overflow-hidden"
              activeOpacity={0.8}
            >
              <LinearGradient colors={['#faf5ff', '#f3e8ff']} start={{x:0,y:0}} end={{x:1,y:1}} className="p-4 flex-row items-center justify-between border border-purple-200 rounded-2xl">
                <View className="flex-row items-center">
          ...
          <TouchableOpacity
            ...
            className="mx-6 bg-white border border-slate-200 p-4 rounded-2xl items-center mb-4 shadow-sm"
            activeOpacity={0.7}
          >
